...
Pour configurer DII, reportez-vous à link:https://docs.netapp.com/us-en/data-infrastructure-insights/task_getting_started_with_cloud_insights.html["Intégration de Data Infrastructure Insights"^].

Notez ce qui suit :

* Si le client crée une nouvelle instance DII, il est recommandé de commencer par un link:https://docs.netapp.com/us-en/data-infrastructure-insights/concept_subscribing_to_cloud_insights.html#trial-edition["Essai gratuit de DII"^]. Pour en savoir plus sur cette fonctionnalité et la liste de vérification requise pour le démarrage, consultez link:https://docs.netapp.com/us-en/data-infrastructure-insights/concept_feature_tutorials.html["Tutoriels sur les fonctionnalités"^].
...
== Voir la consommation de DII

Vous pouvez consulter la consommation de vos services complémentaires DII via la NetApp Console ou Digital Advisor. Les données de consommation des niveaux de service de performance DII (tels que DII Extreme, DII Premium et DII Performance) apparaissent à côté de vos niveaux de service de performance de base dans les vues de consommation actuelle et de tendance de consommation.

L'exemple suivant illustre le niveau de service de performance DII dans l'onglet *Consommation actuelle* dans la NetApp Console :

image:console-current-consumption-dii.png["Capture d'écran de l'onglet de consommation actuelle affichant les niveaux de service DII"]

L'exemple suivant montre les niveaux de service de performance DII dans le widget *Keystone Subscriptions* de Digital Advisor :

image:keystone-widget-dii.png["Capture d'écran du widget Keystone Subscriptions affichant les niveaux de service DII"]

Pour plus d'informations sur la consultation de la consommation, voir :
