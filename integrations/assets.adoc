...
NOTE: La page *Actifs* affiche des onglets en fonction des plateformes associées à vos abonnements Keystone . Pour ONTAP, les onglets *Nœuds dans les clusters* et *Volumes dans les clusters* sont tous deux disponibles. Pour StorageGRID, le tableau *Nœuds dans les grilles* s'affiche directement sans sélection d'onglet. Si l'abonnement inclut les deux plateformes, les trois onglets sont disponibles.
...
+
Vous voyez les informations détaillées sur tous les volumes dans les clusters à travers les abonnements Keystone, y compris le nom du volume, le numéro d'abonnement, le numéro de série du nœud, la conformité aux politiques QoS, le nom du cluster, le nom de l'hôte et les niveaux de service de performance. Vous pouvez surveiller la capacité provisionnée, l'utilisation logique et physique, les données froides et les données chaudes. Vous obtenez également une vue d'ensemble de :
...
NOTE: Les colonnes *Cold tier %* et *Hot tier %* indiquent le pourcentage de données sur chaque niveau, calculé comme (Cold data / Total footprint) × 100 et (Hot data / Total footprint) × 100 respectivement.
