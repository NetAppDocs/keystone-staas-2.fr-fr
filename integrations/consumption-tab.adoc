...
Vous pouvez consulter les prévisions d'utilisation de la capacité pour les 120 prochains jours à compter de la date actuelle grâce à l'option *Forecast* disponible dans la vue *Historical consumption* de l'onglet *Consumption trend* dans la Console. L'option est activée par défaut.

Par défaut, le graphique affiche les 30 derniers jours de consommation historique et une projection sur 120 jours à partir de la date actuelle sur le même graphique. Jusqu'à 30 points de données sont tracés au total (20 pour les données historiques et 10 pour les données de prévision), avec une ligne verticale séparant les deux segments.

image:forecast-1.png["capture d'écran montrant les données de prévision"]

Si vous sélectionnez une période qui ne se termine pas à la date du jour, les prévisions sont automatiquement désactivées. Pour les réactiver, une boîte de dialogue de confirmation vous informe que les données de prévision ne sont disponibles que lorsque la période sélectionnée se termine à la date du jour. La confirmation rétablit l'affichage par défaut, correspondant aux 30 derniers jours de données historiques et aux 120 prochains jours de prévision.

Si la consommation prévue devrait dépasser la capacité de pointe, un indicateur d'alerte apparaît sur le graphique, recommandant une mise à niveau de la capacité.
...
Dans la Console, en plus de la consommation en rafale facturée, vous pouvez également consulter la consommation en rafale non facturée (cumulée mais pas encore facturée) et la consommation en rafale provisoire (cumulée au cours de la période de facturation actuelle et pas encore facturée).
...
Vous pouvez également télécharger les données au format Excel, y compris le statut des rafales (facturées, non facturées ou provisoires) et la facturation par
