...
NOTE: Vous pouvez utiliser le bouton *Prévisions* pour afficher la consommation de capacité projetée dans la vue *Historique de consommation* et le pic de consommation cumulé projeté dans la vue *Pic de consommation cumulé*. Pour en savoir plus, consultez link:../integrations/consumption-tab.html#view-forecast-data["Consulter les données prévisionnelles"].

+

NOTE: Si vous avez souscrit à des services complémentaires tels que protection des données (ADP) ou Data Infrastructure Insights (DII), les données de consommation de ces services apparaissent avec vos niveaux de service de performance de base dans les vues de consommation historique et de pics cumulés. Pour en savoir plus sur les services complémentaires, consultez link:../concepts/adp.html["Découvrez nos services complémentaires"].

+
image:consumption-trend-burst.png["capture d'écran affichant l'historique de consommation"]

+
Le graphique linéaire affiche les données de consommation historiques et permet aux utilisateurs d'analyser les tendances sur une période sélectionnée. Le graphique présente les indicateurs suivants :

+
** *Capacité utilisée* : Capacité de stockage totale utilisée, incluant les données et la capacité des instantanés.
** *Données* : Capacité de stockage utilisée par les données utilisateur.
** *Instantané* : Capacité de stockage utilisée par les données d’instantané.
** *Engagé* : La capacité contractuelle engagée.
** *Capacité de pointe* : La limite de pointe maximale autorisée pour votre abonnement.
** *Dépassement de la capacité de pointe* : Consommation de stockage excédant la limite de pointe. Cette mesure s’affiche uniquement lorsque la limite de pointe est dépassée.
+
En fonction de la plage de dates de votre requête, les graphiques d'utilisation affichent jusqu'à 30 points de collecte de données. Vous pouvez survoler le graphique avec le curseur de votre souris pour afficher la répartition de l'utilisation à chaque point de collecte de données.

+

NOTE: La plage de dates que vous sélectionnez détermine le niveau de détail affiché en bas du graphique. Pour les périodes de 30 jours ou moins, la date et l'heure sont affichées pour chaque point de données. Pour les périodes supérieures à 30 jours, seule la date est affichée.
...
. Sélectionnez la plage de dates à l'aide des icônes de calendrier dans les champs *From Date* et *To Date*. Par défaut, la plage de dates correspond aux 30 derniers jours à compter d'aujourd'hui. Vous pouvez modifier cette plage, mais vous ne pouvez pas sélectionner une date future.
+
...
NOTE: Si vous sélectionnez une période inférieure à 30 jours, le graphique affiche un point de collecte de données par jour uniquement pour les jours sélectionnés. Par exemple, une période de 4 jours affiche 4 points de collecte de données, et non 30. Seule la date est affichée en bas du graphique pour toute période sélectionnée. Les horodatages ne sont visibles que lorsque vous effectuez un zoom avant sur le graphique à l'aide de la fonction de zoom décrite ci-dessous.
...
Dans la console, lorsque l’option *Prévisions* est désactivée, vous pouvez consulter la consommation de pointe facturée, ainsi que la consommation de pointe non facturée (cumulée mais non encore facturée) et la consommation de pointe provisoire (cumulée au cours de la période de facturation actuelle et non encore facturée). Lorsque l’option *Prévisions* est activée, le graphique affiche également la consommation de pointe cumulée projetée pour les 120 prochains jours à compter de la date actuelle. Pour en savoir plus sur l’option *Prévisions*, consultez link:../integrations/consumption-tab.html#view-forecast-data["Consulter les données prévisionnelles"].

image:accr-burst-provision-invoice.png["Capture d'écran montrant l'utilisation cumulée en rafale dans la Console"]
...
== Consulter les données prévisionnelles

Dans la console, vous pouvez utiliser le bouton *Prévisions* de l'onglet *Tendance de consommation* pour afficher l’utilisation de capacité projetée dans la vue *Consommation historique* et le pic cumulé projeté dans la vue *Pic cumulé*. Le bouton est activé par défaut dans les deux vues.

Dans la vue *Historique de consommation*, le graphique affiche les 30 derniers jours de données historiques de consommation et une projection sur 120 jours à partir de la date actuelle sur le même graphique. Jusqu'à 30 points de données sont tracés au total (20 pour les données historiques et 10 pour les données de prévision), avec une ligne verticale séparant les deux segments.

image:forecast-1.png["capture d'écran montrant les données de prévision"]

Dans la vue *Consommation cumulée*, le graphique affiche les 6 derniers mois d’historique de consommation cumulée et une projection sur 120 jours à partir de la date actuelle, avec une ligne verticale séparant les données historiques et les données prévisionnelles. La barre de la période de facturation en cours affiche à la fois l’utilisation provisoire de la consommation cumulée et les données prévisionnelles pour les jours restants. Vous pouvez cliquer sur n’importe quelle barre pour afficher la répartition quotidienne dans le tableau *Consommation cumulée par jour*. Pour en savoir plus sur le tableau *Consommation cumulée par jour*, consultez link:../integrations/consumption-tab.html#view-daily-accrued-burst-data-usage["Afficher l'utilisation quotidienne des données accumulées"].

image:accrued-burst-forecast.png["capture d'écran montrant les prévisions de rafales accumulées"]

Si vous sélectionnez une période ne se terminant pas à la date du jour, les prévisions sont automatiquement désactivées. Pour les réactiver, une boîte de dialogue de confirmation vous informe que les données de prévision ne sont disponibles que lorsque la période sélectionnée se termine à la date du jour. Si vous confirmez la boîte de dialogue, l'affichage revient à la plage par défaut pour la vue sélectionnée : les 30 derniers jours de données historiques plus les 120 prochains jours de prévision dans la vue *Historical consumption*, ou les 6 derniers mois de données historiques de pics de consommation cumulés plus les 120 prochains jours de prévision dans la vue *Accrued burst*.

Si la consommation prévue risque de dépasser la capacité engagée, un indicateur d'alerte orange apparaît sur le graphique, recommandant une augmentation de capacité. Si elle risque de dépasser la capacité de pointe, un indicateur d'alerte rouge apparaît également, recommandant une augmentation de capacité.
...
NOTE: Les données affichées dans le graphique dépendent de l’activation ou de la désactivation de l’option *Prévisions*.

image:bxp-accrued-burst-days-1.png["capture d'écran montrant le tableau des rafales accumulées par jour"]

Vous pouvez également télécharger les données disponibles au format Excel, y compris le statut des rafales (facturées, non facturées, provisoires ou prévisionnelles, selon le cas) et la période de facturation.
...
Vous pouvez également télécharger les données au format Excel, y compris le statut des burst (invoiced, uninvoiced ou provisional) et la période de facturation.
...
* link:../integrations/subscriptions-tab.html["Afficher les détails de vos abonnements Keystone"]
* link:../integrations/current-usage-tab.html["Consultez la consommation actuelle de vos abonnements Keystone"]
* link:../integrations/subscription-timeline.html["Consultez la chronologie de vos abonnements Keystone"]
